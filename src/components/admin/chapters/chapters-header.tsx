  ...
  return (
    <header className="flex items-center gap-3 border-b border-border/50 bg-background/80 backdrop-blur-sm px-4 sm:px-6 py-4 sticky top-0 z-10">
      ...
      <div className="flex-1 min-w-0">
        <h1 className="text-lg font-bold truncate">Chapters</h1>
        <p className="text-xs text-muted-foreground hidden sm:block truncate">
          Atur kurikulum, bab, dan materi untuk:{" "}
          <span className="font-semibold text-foreground">{courseTitle}</span>
        </p>
      </div>
      <Button size="sm" className="shrink-0 gap-1.5" onClick={onAddChapter}>
        <Plus className="h-4 w-4" />
        <span className="hidden sm:inline">Tambah Bab</span>
